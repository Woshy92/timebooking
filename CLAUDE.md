...
- Neues oder geändertes Verhalten wird mit Unit-Tests ausgeliefert (Frontend: Vitest, `npm test` in `frontend/`). Die Acceptance Criteria des Tickets sind das Rückgrat des Testplans.
- User-sichtbare Flows bekommen Playwright-E2E-Abdeckung.
- E2E-Tests müssen **deterministisch** sein: Playwrights Auto-Waiting und pollende `expect`s nutzen, niemals feste Sleeps; von Live-Services (Google Calendar/OAuth) via Fixtures/Mocks isolieren; Testdaten pro Lauf seeden oder zurücksetzen; Role-/Test-ID-Selektoren statt fragiler CSS-Ketten. Grund: eine flaky Suite trainiert alle darauf, Rot zu ignorieren — und eine ignorierte Suite ist schlimmer als keine.
