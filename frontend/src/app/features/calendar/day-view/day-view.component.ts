...
@Component({
  ...
  standalone: true,
  imports: [DurationPipe, FormsModule, ProjectPillsBarComponent, ClearConfirmPopoverComponent, ProjectPopoverComponent, ImportWizardComponent],
  template: `
      ...
      <div class="flex items-center gap-4 px-6 py-3 border-b border-gray-200/80 bg-white sticky top-0 z-20 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
        ...
        @if (googleEvents().length > 0) {
          <button
            ...
            title="Google-Termine importieren"
...
export class DayViewComponent {
  ...
  private readonly viewStart = computed(() => this.ui.viewStartHour());
  ...
  onGridMouseDown(event: MouseEvent) {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const d = this.draft()!;
      this.draft.set({ ...d, endHour: Math.min(Math.max(curHour, d.startHour + 0.25), this.viewEnd()) });
    };
    const onUp = () => {
      stopScroll?.();
  ...
  getDraftTop() { return (this.draft()!.startHour - this.viewStart()) * HOUR_HEIGHT; }
  ...
  getTopPosition(start: Date) { const d = new Date(start); return (d.getHours() + d.getMinutes() / 60 - this.viewStart()) * HOUR_HEIGHT; }
